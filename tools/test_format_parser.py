"""
格式解析器测试 - 验证 Claude Code 与 OpenAI Responses 解析器
"""
import json
...
def test_openai_responses_format():
    """测试 OpenAI Responses 格式解析"""
    print("\n=== 测试 OpenAI Responses 格式 ===")
    ...
    body = {
        ...
        "input": [
            {
                "role": "user",
                "content": [{"type": "input_text", "text": "介绍一下 Python"}],
            }
        ],
        "stream": True,
    ...
    return format_name == "openai_responses"
...
def test_openai_chat_exclusion():
    """测试 OpenAI Chat 格式排斥 OpenAI Responses"""
    print("\n=== 测试 OpenAI Chat 格式排斥 OpenAI Responses ===")
    ...
    if not can_parse:
        print("✅ OpenAI Chat 正确排斥了 OpenAI Responses 格式")
    else:
        print("❌ OpenAI Chat 错误识别了 OpenAI Responses 格式")
...
def test_format_conversion():
    ...
    print("   原始格式: Claude Code")
    print("   转换后: OpenAI Chat")
    print(f"   消息数: {len(openai_body.get('messages', []))}")
    print(f"   模型: {openai_body.get('model')}")

    print("\n2. OpenAI Responses -> Claude Chat")
    responses_body = {
        "model": "gpt-4.1",
        "input": [
            {"role": "user", "content": [{"type": "input_text", "text": "Explain quantum computing"}]}
        ],
    }

    responses_parser = get_parser("openai_responses")
    internal = responses_parser.from_format(responses_body)
    claude_chat_parser = get_parser("claude_chat")
    claude_body = claude_chat_parser.to_format(internal)

    print("   原始格式: OpenAI Responses")
    print("   转换后: Claude Chat")
...
def test_auto_detection():
    ...
    for case in test_cases:
        format_name, _ = detect_and_parse("auto", case["path"], case["headers"], case["body"])
...
def main():
    ...
    tests = [
        ...
        ("OpenAI Responses 格式解析", test_openai_responses_format),
