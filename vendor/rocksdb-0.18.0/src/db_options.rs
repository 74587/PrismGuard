...
impl BlockBasedOptions {
    ...
    pub fn set_lru_cache_compressed(&mut self, size: size_t) {
        let cache = new_cache(size);
        unsafe {
            // Debian/Ubuntu packaged RocksDB C headers do not expose the
            // compressed-cache setter. Fall back to the regular block cache so
            // builds against the system library continue to compile.
            ffi::rocksdb_block_based_options_set_block_cache(self.inner, cache);
        }
    }
